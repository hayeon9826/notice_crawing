...
#원하는 페이지로 이동
@browser.navigate().to "http://www.kinu.or.kr/brd/board/674/L/menu/654?brdType=L&searchField=&searchText=&thisPage=1"

#공지사항 제목 잡기
sleep(1);

@content = @browser.find_elements(css: '#cmsContent > div.board_wrap_bbs.boardTop_txt > table > tbody > tr')


@content.each do |t|
  @title = t.find_element(tag_name: "a").text
  @date = t.find_element(css: "td:nth-child(3)").text
